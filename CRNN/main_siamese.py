...
#from cnn_rnn import *
from data_prepare_siamese import *
...
save_result_dir = "/run/user/1435515045/gvfs/smb-share:server=istanbul.psychology.pitt.edu,share=raw_data/TPOT/Video_Data/Sanchayan2/result_test_siamese/siamese_fold0/"
# training parameters
...
CNN_fc_hidden1, CNN_fc_hidden2 = 1024 ,  768 #-Architecture 1 
...
train_data=np.load('./kfold_data/train_data_reliable_0.npy',allow_pickle=True).item()
valid_data=np.load('./kfold_data/test_data_reliable_0.npy',allow_pickle=True).item()
...
'''
"The next section is just for separate tasks. Uncomment this is you intend to do it for different tasks "
...
for config in configuration_list:
    ...
    cnn_encoder = EncoderCNN(img_x=img_x, img_y=img_y, fc_hidden1=CNN_fc_hidden1, fc_hidden2=CNN_fc_hidden2,
                         drop_p=dropout, CNN_embed_dim=CNN_embed_dim).to(device)
    ...
    if torch.cuda.device_count() > 1:
        ...
        siamese_net = nn.DataParallel(siamese_net,device_ids=[0,1])
