...
class EncoderCNN(nn.Module):
    def __init__(self, img_x=90, img_y=120, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300, num_classes=50):
        ...
        self.temporal_pool= nn.MaxPool1d(90,stride=None)

        self.fc4 = nn.Linear (self.CNN_embed_dim*2, 128)
        self.fc_last = nn.Linear(128,self.num_classes)

    def forward_once(self, x_3d):
        cnn_embed_seq = []
        ...
        # cnn_embed_seq: shape=(batch, time_step, input_size)
       	#pooled = self.temporal_pool(cnn_embed_seq)
       	cnn_embed_seq=cnn_embed_seq.permute(0,2,1).contiguous()
       	pooled= self.temporal_pool (cnn_embed_seq)
        pooled= torch.squeeze(pooled,dim=2)
