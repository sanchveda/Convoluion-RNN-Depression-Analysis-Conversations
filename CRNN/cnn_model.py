...
class CNN_Encoder_Resnet50(nn.Module):
    ...
    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300 , number_of_classes=2):
        ...
        inceptiton_net = InceptionResnetV1(pretrained='vggface2')
        ...
        '''
        for params in inceptiton_net.parameters():
            params.requires_grad=False

        #modules = list(resnet.children())[:-1]      # delete the last fc layer.
        modules_inception = list (inceptiton_net.children())[:-3]
        modules3 = list (inceptiton_net.children())[-1] 

       
        #self.resnet = nn.Sequential(*modules)
        self.inceptiton_net= nn.Sequential (*modules_inception)
        
        self.fc1 = nn.Linear(inceptiton_net.last_linear.in_features, fc_hidden1)
        self.bn1 = nn.BatchNorm1d(fc_hidden1, momentum=0.01)
        self.fc2 = nn.Linear(fc_hidden1, fc_hidden2)
        self.bn2 = nn.BatchNorm1d(fc_hidden2, momentum=0.01)
        self.fc3 = nn.Linear(fc_hidden2, number_of_classes)


    def forward(self, x_3d):

        with torch.no_grad():
            x_3d= self.inceptiton_net (x_3d)
            x = x_3d.view (x_3d.size(0),-1)
